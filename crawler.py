...
def crawl_links(board_name,target_num=1000):
    ...
    while len(links)<target_num:
        if (len(links))==0:
            next, sublist = crawl_page(f"https://www.ptt.cc/bbs/{board_name}/index.html")
        else:
...
def crawl_page(link = "https://www.ptt.cc/bbs/Gossiping/index.html"):
    ...
    for post in soup.find_all('div',class_='title'):
        try:
            title_link[post.a.text]="https://www.ptt.cc/"+post.a['href'] #若文章被刪除會沒有連結
        except:
            pass
    next_page = "https://www.ptt.cc/"+soup.find('a',class_="btn wide",string="‹ 上頁")['href']
...
def crawl_post(link,combine_text=True):
    ...
    for comment in main_content.find_all('div',class_='push'):
        comment_text.append(comment.find('span',class_="f3 push-content").text[1:])
        main_content.find('div',class_='push').extract()
    if combine_text ==True:
